...
> **Use Copilot Studio as the parent/master agent** for the majority of your 100+ agent estate. CS wins on speed, no-code authoring, governance, LOB integration, multi-channel exposure, and factory-scale ALM — all P0 requirements for enterprise delivery at pace. All P1 orchestration patterns (Supervisor, Supervisor-as-tool, Hierarchical) are fully supported in CS. **Introduce Foundry as parent only** for domains explicitly requiring parallel fan-out (Concurrent pattern), dynamic supervisor orchestration (Magentic), complex system prompts beyond 8,000 chars, full distributed tracing, or compliance-grade BYO storage. Avoid unstructured peer-to-peer (network/mesh) patterns on either platform — AgentArch benchmark shows 36% hallucination rate in unstructured multi-agent ReAct vs. 0% in orchestrator-led configurations. Target fewer than 3 Foundry-parent domains out of your total agent estate.
{: .recommendation}
