...
| 📱 Multi-channel exposure (web, mobile) | ✅ **Wins** — native channels include Teams, SharePoint, M365 Copilot, web chat widget, mobile app, custom website, and Direct Line API; zero-config for M365 surfaces | ⚠️ Foundry agents expose an API endpoint only; channel integration (web, mobile, Teams) requires a custom front-end or a CS wrapper to handle channel routing |
