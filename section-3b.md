...
|---|---|---|---|
| 1 | Minimise LLM calls | <span class="adh-full">✅ Full</span> Inline child adds zero extra LLM hop — same session as parent; connected adds one | <span class="adh-partial">🟡 Partial</span> Each Foundry child agent is a separate LLM call; accept only where specialist value is proven |
| 2 | Default to Flows, escalate to Crews | <span class="adh-full">✅ Full</span> Child Topics are deterministic; PA flows handle all tool calls in the child | <span class="adh-partial">🟡 Partial</span> Workflow sub-agent can use deterministic tools; planning only if task is genuinely open-ended |
| 3 | Clarification before action | <span class="adh-full">✅ Full</span> Parent CS Topics handle disambiguation before delegation to child | <span class="adh-partial">🟡 Partial</span> Parent Workflow node or Magentic manager requests clarification; requires explicit design |
| 4 | External verification — never self-check | <span class="adh-full">✅ Full</span> Parent reads back typed I/O output from child; deterministic read-back, not agent self-check | <span class="adh-full">✅ Full</span> Parent validates child JSON output schema before passing downstream; structured output enables checks |
| 5 | Externalize termination logic | <span class="adh-full">✅ Full</span> Parent Topic conditions + session chain limits (100 turns, 60 min) for full parent+child chain | <span class="adh-full">✅ Full</span> Workflow termination nodes + SDK `max_steps` per child; never in LLM prompt |
| 6 | Tool argument quality | <span class="adh-full">✅ Full</span> Typed YAML I/O for connected agents; shared parent connector for inline; structured error responses | <span class="adh-full">✅ Full</span> Structured JSON schema per child; argument validation at each hop; App Insights flags failures |
| 7 | Consolidate tools | <span class="adh-full">✅ Full</span> Inline child uses parent's PA connectors — no duplication; connected child has own connector set | <span class="adh-partial">🟡 Partial</span> Each Foundry child has own tool set; consolidation is an explicit SDK design decision |
| 8 | Multi-agent is a reliability tax | <span class="adh-full">✅ Full</span> Inline child: zero extra hop; connected: one extra hop — each adds reliability cost and is justified | <span class="adh-partial">🟡 Partial</span> Each Workflow node adds one LLM inference cost; specialisation value must outweigh reliability cost |
| 9 | Context hygiene at scale | <span class="adh-partial">🟡 Partial</span> Inline: parent context shared; connected: own session, correlationId links; no automatic compaction | <span class="adh-full">✅ Full</span> Per-thread context per child; explicit state passed; compaction in SDK code |
| 10 | Sensitive state in side channel | <span class="adh-full">✅ Full</span> Inline child inherits parent Key Vault / PA credentials; connected uses own Entra connection | <span class="adh-full">✅ Full</span> Managed Identity per child; credentials never in Workflow parameters or child system prompt |
| 11 | Tool accuracy > 90% as deployment gate | <span class="adh-full">✅ Full</span> CoE deployment gate per child agent before PP Pipelines promotion | <span class="adh-full">✅ Full</span> App Insights evaluation threshold per child before production promotion |
| 12 | Human-in-the-loop for irreversible actions | <span class="adh-full">✅ Full</span> PA approval flow in parent or child before irreversible writes; parent waits for confirmation | <span class="adh-full">✅ Full</span> HITL Workflow node; parent Workflow pauses for approval before triggering next child |
| 13 | Use lowest complexity | <span class="adh-full">✅ Full</span> Inline child is lowest complexity option for CS-native tasks; zero extra infra | <span class="adh-maybe">🔶 Maybe</span> Infrastructure overhead; accept only where Foundry capabilities (parallel, Magentic, JSON schema) are required |
| 14 | Justify every agent boundary | <span class="adh-full">✅ Full</span> Each child scoped to one domain or compliance boundary; Gateway → Domain → Specialist enforced | <span class="adh-full">✅ Full</span> Each Workflow sub-agent boundary justified by specialization need; no boundary without demonstrated failure reason |
| 15 | Single responsibility per agent | <span class="adh-full">✅ Full</span> Each CS child has a narrow domain; CoE Toolkit monitors for overlap | <span class="adh-full">✅ Full</span> Each Foundry child has one specialist task; App Insights flags redundant invocations |
| 16 | Right-size the model per agent role | <span class="adh-no">❌ No</span> Child model locked to Microsoft-managed — no per-child tuning or model selection | <span class="adh-full">✅ Full</span> Per-child model selection; smaller, cheaper models for extraction or formatting children |
| 17 | Validate output before passing downstream | <span class="adh-partial">🟡 Partial</span> Typed YAML I/O for connected agents; Topic conditions on inline; inline relies on LLM judgment | <span class="adh-full">✅ Full</span> JSON schema validation per child; parent orchestrator halts or retries on malformed output |
| 18 | Surface errors explicitly | <span class="adh-full">✅ Full</span> Error Topics in CS child; connector structured error responses; parent handles child errors | <span class="adh-full">✅ Full</span> SDK error handling per child; errors propagated to parent orchestrator; never swallowed |
| 19 | Idempotency for all writes | <span class="adh-full">✅ Full</span> Idempotency key in child PA flows; safe to re-trigger | <span class="adh-full">✅ Full</span> Idempotency key in child Azure Functions or OpenAPI tool definitions |
| 20 | Kill switch | <span class="adh-full">✅ Full</span> Child agent disabled in CS portal independently of parent; no redeployment | <span class="adh-full">✅ Full</span> Foundry child disabled in Foundry portal; parent Workflow fails gracefully or reroutes |
| 21 | Security trimming in every agent | <span class="adh-full">✅ Full</span> Dataverse RLS per child CS agent; child cannot return data above user entitlement | <span class="adh-full">✅ Full</span> Managed Identity RBAC per Foundry child; trimming enforced in each child's response logic |
| 22 | Content safety at every layer | <span class="adh-full">✅ Full</span> AI Builder content safety filter per child CS agent; applied at input and output | <span class="adh-full">✅ Full</span> Azure AI Content Safety filter per Foundry child; applied at every agent boundary |
| 23 | Scope agent authority to operations | <span class="adh-partial">🟡 Partial</span> Topic-level permissions per child; PA connector scopes limited but no blast radius caps | <span class="adh-full">✅ Full</span> RBAC per child; blast radius caps enforced in SDK orchestration code |
| 24 | Instrument all handoffs | <span class="adh-partial">🟡 Partial</span> correlationId propagated from parent to each child; Dataverse logs; limited deep-chain tracing | <span class="adh-full">✅ Full</span> Full OpenTelemetry; every parent-child message, tool call, and handoff traceable in App Insights |
| 25 | Automated evaluation in CI/CD | <span class="adh-full">✅ Full</span> PP Pipelines with solution validation per child agent before promotion | <span class="adh-full">✅ Full</span> Azure DevOps / GitHub Actions per Foundry child; evaluation step before production |
| 26 | Evaluate continuously in production | <span class="adh-full">✅ Full</span> CoE Toolkit monitoring per child agent; intent resolution and tool accuracy dashboards | <span class="adh-full">✅ Full</span> App Insights dashboards per child; custom evaluation queries per specialist |
| 27 | Maintain agent inventory | <span class="adh-full">✅ Full</span> Agent 365 tracks all CS child agents; registered, owned, versioned | <span class="adh-maybe">🔶 Maybe</span> No native registry for Foundry children; custom Azure Policy + Entra tracking required |
| 28 | Externalize and version-control prompts | <span class="adh-full">✅ Full</span> Child agent instructions in CS solution; promoted via PP Pipelines with controlled rollout | <span class="adh-full">✅ Full</span> Child prompts in Azure DevOps; versioned separately from parent Workflow YAML |
